...
class Frontier_Explorer(Node):
    ...
    def explore(self):
        ...
        # use np.where to find unknown mask which is indicated by the value -1
        unknown_mask = np.where(self.occupancy.probs == -1, 1, 0)

        # use np.where to find unknown mask which is indicated by values in range [0, 0.5)
        ...
        frontier_mask = np.where(
            (unoccupied >= 0.3 * total) & (unknown >= 0.2 * total) & (occupied == 0),
            1,
            0
        )
